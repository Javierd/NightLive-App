...
public class LocationUpdatesService extends Service{
    ...
    private static final String PACKAGE_NAME =
            "com.javierd.nightlive.locationupdatesforegroundservice";
    ...
    /**
     * Upload the new location to our server
     * @param location The {@link Location} to upload.
     * @param pub The {@link int} that determines if it is public (1) or not (0).
     * */
    protected void postLocation(Location location, int pub){
        ...
        String userName = PreferenceManager.getDefaultSharedPreferences(this).getString(LoginActivity.USER_NAME, "None");
        String userToken = PreferenceManager.getDefaultSharedPreferences(this).getString(LoginActivity.USER_TOKEN, "None");

        Call<Integer> call = apiService.postUserLocation(location.getLatitude(), location.getLongitude(), userName, userToken, pub);
        call.enqueue(new Callback<Integer>() {
            @Override
            public void onResponse(@NonNull Call<Integer> call, @NonNull Response<Integer> response) {
                int statusCode = response.code();

                String text = getString(R.string.unexpected_error);
                switch(statusCode){
                    case 400:
                        //Bad request
                        text = getString(R.string.wrong_server_direction);
                        return;
                    case 401:
                        //Unauthorized
                        PreferenceManager.getDefaultSharedPreferences(getBaseContext()).edit().putBoolean(LoginActivity.SIGNED_IN_USER, false).apply();
                        break;
                }
                ...
                switch (result){
                    case 0:
                        text = "Location uploaded correctly";
                        break;
                }
                Log.i("Location sent result", text);
            }

            @Override
            public void onFailure(@NonNull Call<Integer> call, @NonNull Throwable t) {
                // Log error here since request failed
                Log.i("Error sending location", String.valueOf(t));
            }
        });
    }
